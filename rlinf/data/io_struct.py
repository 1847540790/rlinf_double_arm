...
class EnvOutput:
    ...
    def prepare_observations_double_arm(self, obs: dict[str, Any]) -> dict[str, Any]:
        """
        Prepare observations for dual arm environments.
        
        Args:
            obs: Observation dict containing:
                - left_wrist_images: Left arm wrist camera images
                - right_wrist_images: Right arm wrist camera images
                - base_images: Optional base/table camera images
                - states: Robot state vector (14 dims: 7 per arm)
                - task_descriptions: Task description strings
        
        Returns:
            Processed observation dict for dual arm format
        """
        left_wrist_images = obs["left_wrist_images"] if "left_wrist_images" in obs else None
        right_wrist_images = obs["right_wrist_images"] if "right_wrist_images" in obs else None
        base_images = obs["base_images"] if "base_images" in obs else None
        states = obs["states"] if "states" in obs else None
        task_descriptions = (
            list(obs["task_descriptions"]) if "task_descriptions" in obs else None
        )
    ...
    def _is_dual_arm(self, obs: dict[str, Any]) -> bool:
        """
        Check if the observation is from a dual arm environment.
        
        Args:
            obs: Observation dict
        
        Returns:
            True if dual arm format (has left_wrist_images and right_wrist_images), False otherwise
        """
        return "left_wrist_images" in obs and "right_wrist_images" in obs

    def to_dict(self):
        env_output_dict = {}

        # Check if dual arm format and use appropriate preparation method
        if self._is_dual_arm(self.obs):
            env_output_dict["obs"] = self.prepare_observations_double_arm(self.obs)
        else:
            env_output_dict["obs"] = self.prepare_observations(self.obs)
        
        if self.final_obs is not None:
            if self._is_dual_arm(self.final_obs):
                env_output_dict["final_obs"] = self.prepare_observations_double_arm(self.final_obs)
            else:
                env_output_dict["final_obs"] = self.prepare_observations(self.final_obs)
        else:
            env_output_dict["final_obs"] = None
