...
class OpenPi0ForRLActionPrediction(PI0Pytorch):
    ...
    def input_transform(self, obs: dict, transpose=True):
        # print(f"[DEBUG] input_transform: entry, obs keys={list(obs.keys()) if isinstance(obs, dict) else type(obs)}")
        
        inputs = jax.tree.map(lambda x: x, obs)
        # process input
        first_process = "prompt" in inputs.keys()
        # Detect dual arm format (has left_wrist_images and right_wrist_images)
        is_dual_arm = "left_wrist_images" in inputs and "right_wrist_images" in inputs
        # print(f"[DEBUG] input_transform: first_process={first_process}, is_dual_arm={is_dual_arm}")
        if first_process:
            inputs.pop("prompt")
        elif is_dual_arm:
            # Dual arm recompute path - keep dual arm keys, filter out non-observation keys
            dual_arm_keys = {"left_wrist_images", "right_wrist_images", "states", "base_images"}
            inputs = {key: inputs[key] for key in inputs.keys() if key in dual_arm_keys}
        else:
            inputs = {key: inputs[key] for key in inputs.keys() if "/" in key}
        # print(f"[DEBUG] input_transform: after filtering, inputs keys={list(inputs.keys())}, len={len(inputs)}")
        # tensor -> numpy
        inputs = jax.tree.map(
            lambda x: np.asarray(x.detach().cpu()) if torch.is_tensor(x) else x, inputs
        )
        # print(f"[DEBUG] input_transform: after jax.tree.map, inputs keys={list(inputs.keys())}")
        # for k, v in inputs.items():
        #     print(f"[DEBUG] input_transform: {k} type={type(v)}, has_shape={hasattr(v, 'shape')}, shape={getattr(v, 'shape', 'N/A')}")
    ...
    def obs_processor(self, env_obs):
        """
        Convert environment observations to policy input format.

        Supports three observation formats:
        1. Single arm format: images, wrist_images, states, task_descriptions
        2. Dual arm format: left_wrist_images, right_wrist_images, states, task_descriptions
        3. CALVIN format: images, states (split into ee_pos, ee_rot, gripper)

        Args:
            env_obs: Environment observation dict

        Returns:
            Processed observation dict for the model
        """
        # print(f"[DEBUG] obs_processor: entry, env_obs keys={list(env_obs.keys())}")
        # Detect dual arm format (has left_wrist_images and right_wrist_images)
        is_dual_arm = "left_wrist_images" in env_obs and "right_wrist_images" in env_obs
        if is_dual_arm:
            # Dual arm format: left_wrist_images, right_wrist_images, states
            # Use "prompt" key to trigger first_process path in input_transform
            processed_obs = {
                "left_wrist_images": env_obs["left_wrist_images"],
                "right_wrist_images": env_obs["right_wrist_images"],
                "states": env_obs["states"],
                "prompt": env_obs["task_descriptions"],  # Use "prompt" for input_transform compatibility
            }
            # Optional base camera
        ...
        # Single arm format: images, wrist_images, states
        processed_obs = {
