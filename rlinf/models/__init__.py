    ...
    if cfg.model_name == "openvla":
        ...
        if simulator_type == "libero":
            ...
            if getattr(cfg.openpi, "pi05", False):
                ...
                actor_train_config = get_openpi_config("pi0_metaworld")
        elif simulator_type == "isaaclab_remote_double":
            if getattr(cfg.openpi, "pi05", False):
                actor_train_config = get_openpi_config("pi05_metaworld")
            else:
                actor_train_config = get_openpi_config("pi0_metaworld")
        ...
        # Override asset_id from user config if provided
        if hasattr(cfg.openpi, "asset_id") and cfg.openpi.asset_id is not None:
            import dataclasses
            data_config = dataclasses.replace(data_config, asset_id=cfg.openpi.asset_id)
        norm_stats = None
        ...
        # For dual arm environments, replace data_transforms with DualArmInputs/DualArmOutputs
        is_dual_arm = simulator_type == "isaaclab_remote_double" or simulator_type == "realworld"
        if is_dual_arm:
            from rlinf.models.embodiment.openpi.policies.dual_arm_policy import (
                DualArmInputs,
                DualArmOutputs,
            )
            # Get action_dim from config (should be 14 for dual arm)
            action_dim = getattr(cfg, "action_dim", 14)
            # Replace data_transforms with dual arm transforms
            dual_arm_input_transform = DualArmInputs(model_type=actor_model_config.model_type)
            dual_arm_output_transform = DualArmOutputs(action_dim=action_dim)
            model.setup_wrappers(
                transforms=[
                    *repack_transforms.inputs,
                    transforms.InjectDefaultPrompt(default_prompt),
                    dual_arm_input_transform,  # Use DualArmInputs instead of data_config.data_transforms.inputs
                ...
                output_transforms=[
                    *data_config.model_transforms.outputs,
                    ...
                    dual_arm_output_transform,  # Use DualArmOutputs instead of data_config.data_transforms.outputs
