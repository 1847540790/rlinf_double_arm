...
def get_env_cls(simulator_type, env_cfg=None):
    ...
    if simulator_type == "maniskill":
        ...
        from rlinf.envs.isaaclab_remote_double_arm import REGISTER_REMOTE_ISAACLAB_ENVS

        if env_cfg is None:
            raise ValueError(
                "env_cfg is required for isaaclab_remote_double simulator type. "
                "Please provide env_cfg.init_params.id to select the task."
            )

        task_id = env_cfg.init_params.id
        assert task_id in REGISTER_REMOTE_ISAACLAB_ENVS, (
            f"Task type {task_id} has not been registered! "
            f"Available tasks: {list(REGISTER_REMOTE_ISAACLAB_ENVS.keys())}"
        )
        return REGISTER_REMOTE_ISAACLAB_ENVS[task_id]
        ...
        return CalvinEnv
    elif simulator_type == "flexiv":
        from rlinf.envs.real_world.flexiv_env import FlexivRealEnv

        return FlexivRealEnv
    elif simulator_type == "nmxrdk":
        from rlinf.envs.real_world.nmxrdk_env import NmxRdkRealEnv

        return NmxRdkRealEnv
    else:
