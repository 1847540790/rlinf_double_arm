    ...
    def load_video_data(self) -> bool:
        ...
        try:
            ...
            with h5py.File(self.umi_video_path, 'r') as h5_file:
                ...
                umi_data = h5_file[self.umi_camera_key][:]
                # Convert RGB to BGR if needed (HDF5 typically stores RGB, OpenCV needs BGR)
                if len(umi_data.shape) == 4 and umi_data.shape[3] == 3:
                    self.umi_video_data = cv2.cvtColor(umi_data, cv2.COLOR_RGB2BGR) if umi_data.shape[0] == 1 else np.array([cv2.cvtColor(frame, cv2.COLOR_RGB2BGR) for frame in umi_data])
                else:
                    self.umi_video_data = umi_data
            ...
            with h5py.File(self.robot_video_path, 'r') as h5_file:
                ...
                robot_data = h5_file[self.robot_camera_key][:]
                # Convert RGB to BGR if needed (HDF5 typically stores RGB, OpenCV needs BGR)
                if len(robot_data.shape) == 4 and robot_data.shape[3] == 3:
                    self.robot_video_data = cv2.cvtColor(robot_data, cv2.COLOR_RGB2BGR) if robot_data.shape[0] == 1 else np.array([cv2.cvtColor(frame, cv2.COLOR_RGB2BGR) for frame in robot_data])
                else:
                    self.robot_video_data = robot_data
