...
class AutoActionManagerV2(BaseChunkManager):
    """
    ...
    Args:
        ...
        direction_consistency_range (int): Number of steps to check for direction consistency after
                                           optimal start index. The search_end will be reduced by this
                                           value to ensure enough actions remain for consistency check.
                                           (default: 5)
    """
    ...
    def __init__(
            ...
            direction_consistency_range: int = 5
    ):
        super().__init__()
        ...
        self.direction_consistency_range = direction_consistency_range

        # Calculate dt (time per step) in seconds
        self.dt_seconds = 1.0 / execution_fps

        # Chunk counter (starts from 0, first chunk will be 1)
        ...
        logger.info(f"AutoActionManager initialized: execution_fps={execution_fps}Hz, "
                    ...
                    f"direction_consistency_range={direction_consistency_range}, "
    ...
    def put(
        ...
        # Increment chunk counter and log
        self.chunk_counter += 1
        logger.info(f"AutoActionManager: Processing action chunk #{self.chunk_counter}, length={chunk_length}")
        ...
        current_executing_action = self._get_current_executing_action()  # current state
        device_names = list(action_chunk_dict.keys())
        logger.info(f"action_chunk_dict[0:10]{action_chunk_dict[device_names[0]]}")
        if current_executing_action is None:
            raise ValueError("None Action")
            logger.warning(
                "AutoActionManager: Cannot get current executing action, using estimated_steps_elapsed as start index")
            ...
            if estimated_steps_elapsed < chunk_length:
                ...
                logger.warning(
                    f"AutoActionManager: Estimated steps {estimated_steps_elapsed} >= chunk_length {chunk_length}, "
                    f"using start index 0")
                optimal_start_index = 0
        else:
            # Search for optimal start index
            # If estimated_steps_elapsed >= chunk_length, search the entire chunk due to severe delay
            optimal_start_index = self._find_optimal_start_index(
                ...
                direction_consistency_range=self.direction_consistency_range
            )
            if optimal_start_index is None:
                logger.warning(f"AutoActionManager: optimal_start_index is None, skipping")
                return

        # Store new chunk and set current_step to optimal_start_index
        # Deep copy to avoid shared memory reference issues
        new_action_chunk_dict = {k: v.copy()[optimal_start_index:,] for k, v in action_chunk_dict.items()}

        # Apply direction consistency check if enabled
        direction_consistency_range = self.direction_consistency_range
        if direction_consistency_range > 0:
            all_actions = [current_executing_action, ] + [{k: v[i] for k, v in new_action_chunk_dict.items()} for i in
                                                          range(direction_consistency_range)]
            all_device_names = list(new_action_chunk_dict.keys())

            # Determine action dimension and xyz indices based on concatenated action format
            sample_action = np.concatenate([all_actions[0][dev] for dev in all_device_names], axis=0)
            action_dim = len(sample_action)

            if action_dim == 16:
                # Dual-arm data: Left arm xyz [0,1,2], Right arm xyz [8,9,10]
                xyz_indices = [0, 1, 2, 8, 9, 10]
            elif action_dim == 8:
                # Single arm data: xyz [0,1,2]
                xyz_indices = [0, 1, 2]
            else:
                # Other format: skip direction consistency check
                logger.warning(
                    f"AutoActionManager: Unsupported action dimension {action_dim} for direction consistency check, skipping")
                xyz_indices = None

            if xyz_indices is not None:
                # Extract xyz components from concatenated actions
                all_actions_xyz = [np.concatenate([a[dev] for dev in all_device_names], axis=0)[xyz_indices] for a in
                                   all_actions]
                total_direction = all_actions_xyz[-1] - all_actions_xyz[1]
                for i in range(direction_consistency_range - 1):
                    # Check if current direction is consistent with total direction
                    crt_direction = all_actions_xyz[i + 1] - all_actions_xyz[i]
                    consistency_variable = crt_direction * total_direction
                    neg_direction_indices = consistency_variable < 0
                    all_actions_xyz[i + 1][neg_direction_indices] = all_actions_xyz[i][neg_direction_indices]
                new_actions_xyz = all_actions_xyz[1:]

                # Update the action chunk dict with corrected xyz values
                # Need to map back xyz_indices to individual device actions
                for i in range(direction_consistency_range - 1):
                    offset = 0
                    for device_name in all_device_names:
                        device_action_dim = len(new_action_chunk_dict[device_name][i])
                        # Find which xyz_indices belong to this device
                        device_xyz_indices = [idx - offset for idx in xyz_indices if
                                              offset <= idx < offset + device_action_dim]
                        # Find corresponding positions in new_actions_xyz
                        xyz_positions = [j for j, idx in enumerate(xyz_indices) if
                                         offset <= idx < offset + device_action_dim]
                        for local_idx, xyz_pos in zip(device_xyz_indices, xyz_positions):
                            new_action_chunk_dict[device_name][i][local_idx] = new_actions_xyz[i][xyz_pos]
                        offset += device_action_dim
        self.current_action_chunk_dict = new_action_chunk_dict
        self.last_read_time = observation_timestamp
        self.current_chunk_timestamp = observation_timestamp if observation_timestamp is not None else 0
        self.current_step = 0

        logger.info(
            f"AutoActionManager: Chunk #{self.chunk_counter} stored with optimal start index={optimal_start_index}, "
    ...
    def _get_current_executing_action(self) -> Optional[Dict[str, np.ndarray]]:
        ...
        logger.info(f"shm_now_action_chunk[0:10]{first_chunk}")
        chunk_length = len(first_chunk)
    ...
    def _find_optimal_start_index(
            ...
            direction_consistency_range: int = 0
    ) -> int:
        """
        ...
        If estimated_steps_elapsed >= chunk_length, indicates severe delay, and the entire chunk
        will be searched instead of a limited range around estimated_steps_elapsed.
        ...
        Args:
            ...
            search_tolerance: Tolerance for search range (only used when estimated_steps_elapsed < chunk_length)
            ...
            chunk_length: Length of the action chunk
            direction_consistency_range: Number of steps reserved for direction consistency check.
                                         search_end will be reduced by this value to avoid boundary issues.
        ...
        try:
            ...
            # Use provided chunk_length if available, otherwise calculate from first device
            if chunk_length is None:
                chunk_length = len(action_chunk_dict[first_device])

            # Ensure chunk_length is valid
            if chunk_length is None or chunk_length <= 0:
                logger.error(f"AutoActionManager: Invalid chunk_length={chunk_length}, using index 0")
                return None

            # Calculate max_search_end to reserve space for direction consistency check
            # This ensures that after slicing from optimal_start_index, there are enough actions
            # for direction consistency processing
            max_search_end = chunk_length - 1 - direction_consistency_range
            if max_search_end < 0:
                logger.warning(
                    f"AutoActionManager: chunk_length={chunk_length} is too small for "
                    f"direction_consistency_range={direction_consistency_range}, setting max_search_end=0")
                max_search_end = 0

            # If estimated_steps_elapsed >= chunk_length, delay is severe, search entire available chunk
            if estimated_steps_elapsed >= chunk_length:
                search_start = 0
                search_end = max_search_end
                logger.warning(
                    f"AutoActionManager: Severe delay detected (estimated_steps_elapsed={estimated_steps_elapsed} "
                    f">= chunk_length={chunk_length}), searching entire available chunk [0, {max_search_end}]")
            else:
                ...
                search_end = min(max_search_end, estimated_steps_elapsed + search_tolerance)
            ...
            best_indices = []
            min_distances = []
            logger.info(f"Num of deivces: {len(device_names)}")
            ...
            # else:
            #     logger.warning(
            #         "AutoActionManager: No valid devices found for optimal index calculation, using search_start")
            #     return search_start
